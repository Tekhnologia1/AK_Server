...
const RouteForm = ({ data = {}, handleSubmit, isEditMode, className }) => {
  ...
  const [values, setValues] = useState({
    ...
    // area: data.areas_id || [],
    // area: data.areas_id ? transformedarea.filter((item) => data.areas_id.includes(item.option)) : [],
  ...
  useEffect(() => {
    if (data) {
      const areasIdArray = data.areas_id ? data.areas_id
      .replace(/[\[\]]/g, '') // Remove square brackets
      .split(',')             // Split by comma
      .map(Number) : []; // Converts "12" to [12]
      const result = data.areas_id ? transformedarea.filter((item) => {
...
//   <MultiSelectComponent
//     label={isEditMode ? "Area Covered" : ""}
//     value={transformedarea.filter((area) => values.area.includes(area.option))}
//     filterPlaceholder="Select Area"
//     options={transformedarea}
//     search={true}
//     name="area"
//     onChange={handleMultiSelectChange} // Use a dedicated handler for multi-select
//     placeholder={"Area Covered"}
//   />
//   {errors.area && <p className="text-danger">{errors.area}</p>}
// </div> */}
